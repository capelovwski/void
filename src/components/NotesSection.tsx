...
import { NotebookPen, Plus, Pencil, Trash2, Check, X, CircleAlert } from 'lucide-react';
import { useAuth } from '../contexts/AuthContext';
import { useNotes } from '../hooks/useNotes';

// Bloco de anotações usado dentro do Planejamento. Antes era uma aba
// própria (NotesTab); virou seção para a navegação ficar com 5 itens.
export const NotesSection: React.FC = () => {
  const { user } = useAuth();
  const { notes, loading, error, addNote, updateNote, deleteNote } = useNotes(user?.uid ?? null);
  ...
  return (
    <div className="card-premium p-6 space-y-6">
      <div className="flex items-center gap-2 text-neutral-11 border-b border-neutral-02 pb-3">
        ...
        <h2 className="text-lg font-bold font-albert-sans">Anotações</h2>
      </div>
